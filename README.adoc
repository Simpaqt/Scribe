...
* *`-H` / `--here`* on the CLI applies to every command (TUI, `--new`,
  `--list`, `--export`).
* *Inside the TUI*, creation always lands in the directory you're currently
  listing. Use *`c`* to open the directory picker (recent dirs + free path
  entry), or *`H`* to jump the listing to your current shell directory.
  Then `i` / `t` create the new doc there.
...
| `i`       | Create a new document in the currently listed directory
...
* Linux/macOS: `nvim`
* Windows: tried `code`, `notepad++`, then `notepad`
